package activity.result;

import java.util.ArrayList;
import java.util.List;

public class AddToWatchedListResult {

    private final List<Object> modelsList;

    public AddToWatchedListResult(List<Object> modelsList) {
        this.modelsList = modelsList;
    }

    public List<Object> getModelsList() {
        return modelsList;
    }

    @Override
    public String toString() {
        return "AddToWatchedListResult{" +
                "modelsList=" + modelsList +
                '}';
    }

    //CHECKSTYLE:OFF:Builder
    public static Builder builder() {
        return new Builder();
    }

    public static class Builder {
        private List<Object> modelsList;
        public Builder withList(List<Object> list) {
            this.modelsList = new ArrayList<>(list);
            return this;
        }
        public AddToWatchedListResult build() {
            return new AddToWatchedListResult(modelsList);
        }
    }
}
